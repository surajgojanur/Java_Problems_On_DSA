class Solution {
    ...
    private void dfs(int row,int col,int rows,int cols,boolean[][] visited,int preh,int[][] heights){
        if(row<0 || row>=rows || col<0 || col>=cols || visited[row][col] || preh>heights[row][col])
        return;
